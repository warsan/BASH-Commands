...
ls -l                   # отображать расширенную информацию о файлах и папках
...
## Конструкция CASE  
Если вы столкнулись с парой различных возможных действий, то использование оператора case может быть более полезным, чем вложенные операторы if.
Для более сложных условий используйте пример, как показано ниже
...
Двойной амперсанд (И) **&&** - вторая команды будет выполнена только в случае УСПЕШНОГО завершения первой команды

```bash
# команда2 будет выполнена тогда и только тогда, когда команда1 завершится успешно (возвращает статус выхода 0)
command1 && command2
```

Двойной пайп (ИЛИ) - вторая команды будет выполнена только в случае НЕУДАЧНОГО завершения первой команды

```bash
# команда2 будет выполнена, если, и только если команда1 завершится неудачно (возвращается код ошибки)
command1 || command2
```  
